...
fn synth_events() -> Vec<Event> {
    // Region "bootstrap" on tid 1 over [1000, 2000) µs; "clean" on tid 2 over
    ...
    let json = r#"[
      ...
      {"name":"worker.bootstrap","ph":"B","ts":1000,"pid":1,"tid":1},
      {"name":"worker.bootstrap","ph":"E","ts":2000,"pid":1,"tid":1},
      {"name":"worker.clean_inflate","ph":"B","ts":2000,"pid":1,"tid":2},
      {"name":"worker.clean_inflate","ph":"E","ts":3000,"pid":1,"tid":2}
    ]"#;
...
fn samples_bucket_into_the_correct_region_window() {
    ...
    // bootstrap window [1000,2000): 3 DRAM + 1 L1.  clean [2000,3000): 4 L1.
    let mem = vec![
    ...
    let region_funcs = vec![
        ("bootstrap".to_string(), vec!["bootstrap".to_string()]),
        ("clean".to_string(), vec!["clean_inflate".to_string()]),
    ];
    let rows = rollup(&events, &mem, &[], &region_funcs);
    let boot = rows.iter().find(|r| r.region == "bootstrap").unwrap();
    let clean = rows.iter().find(|r| r.region == "clean").unwrap();

    assert_eq!(boot.mem_samples, 4, "bootstrap should capture its 4 samples");
    assert_eq!(boot.dram, 3);
    assert!(
        (boot.dram_frac() - 0.75).abs() < 1e-6,
        "bootstrap DRAM frac = 3/4"
    );
    assert_eq!(clean.mem_samples, 4, "clean should capture its 4 L1 samples");
    assert_eq!(clean.dram, 0);
    assert!((clean.l1_frac() - 1.0).abs() < 1e-6);

    // Windows are disjoint → concurrency 0 for both.
    assert!(boot.concurrency < 1e-9 && clean.concurrency < 1e-9);

    // Mean load cycles: bootstrap (DRAM-heavy) must be far above clean (all L1).
    assert!(
        boot.mean_load_cycles() > clean.mean_load_cycles() * 5.0,
        "DRAM-bound region must show much higher modeled load latency: {} vs {}",
        boot.mean_load_cycles(),
        clean.mean_load_cycles()
    );
}
...
fn counter_intervals_attribute_by_overlap_and_give_ipc_mpki() {
    ...
    // it overlaps bootstrap and clean equally (1000µs each) → 50/50 split.
