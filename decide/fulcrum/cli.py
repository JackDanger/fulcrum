...
gzippy ships one):
  ...
  (the composed five-gate flow moved to the all-Rust in-process binary:
   `fulcrum run <spec> --gate` — PROVENANCE → DIMENSIONED-QUANTITY →
   PERTURBATION → COMPARABILITY → FINDING-STORE → banked cell_id, no subprocess.)
  ledger [path]
...
# NOTE: the `pipeline` subcommand (the subprocess-seam five-gate flow over
# core/pipeline.py + core/cell.py) was REMOVED — that flow is now the all-Rust
# in-process `fulcrum run <spec> --gate` (src/pipeline.rs). The remaining Python
# here is a dev *oracle* of the un-ported analysis engines (locate/cycles/insn/
# decide/ledger/adapter), not a production path.
