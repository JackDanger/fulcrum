...
| `fulcrum try <ref>` | **Is this change good?** | The whole promotion evaluation: builds both arms from git refs in throwaway worktrees (stale controls impossible, NO-OPs refused on identical binary hashes), runs `verify`, runs the size + paired wall censuses on BOTH arms at a level set that must span shallow (≤4) and deep (≥6) — single-level verdicts are REFUSED — then applies `docs/promotion-rule.md` clause by clause. Clause 5 is the **margin-floor rule** (2026-08-10 redesign): wall convictions — beyond-budget erosions AND pass→fail flips — require cross-layout **CONFIRMED-REAL** (`try` auto-runs `layout confirm` per suspect coordinate, capped at 12/run, overflow stays UNDECIDED); a confirmed-real erosion on a winning cell (base ≤ 0.80) is acceptable while post ≤ min(0.80, 1 − 3·layout_floor); thin margins (base > 0.80) keep the flat 0.005 budget; size cells are exact and unchanged. Output: **SHIP / NO-SHIP (failed clause + numbers) / UNDECIDED (exactly what to re-run)** — each wall suspect prints its full chain: census → floor screen → confirm verdict → margin-floor arithmetic. Missing architectures ⇒ UNDECIDED, never a single-arch SHIP. |
