...
//! profiles *competing implementations* (the tool under test plus any number of
//! alternative tools that do the same job) at comparable granularity on the
//! SAME inputs, so a lever recommendation can say "tool A spends 34% in memory
//! stalls here; tool B spends 11% — the gap is real and here" instead of "I
//! imagine tool B is faster because X".
//!
...
//!
//! Absolute throughput differs (it's the thing under test). What's comparable
//! is the TMA shape and the top hot-function buckets normalized to 100% —
//! "tool B is 70% retiring on a given workload; tool A is 45% retiring / 33%
//! backend" pinpoints whether the loss is wasted work (low retiring) or a
//! slower algorithm doing the same work (high retiring, lower throughput).
...
pub fn render_comparison(input: &str, profiles: &[ToolProfile]) -> String {
    ...
    // A focused "where the subject differs" diff. The FIRST profile is treated
    // as the tool under test (the subject); every other tool is diffed against
    // it, so the reader sees exactly where the subject's TMA/bucket shape
    // diverges from each alternative.
    if let Some((subject, rest)) = profiles.split_first() {
        for other in rest {
            s.push_str(&format!(
                "\n  {} − {}: backend {:+.0}pp | bad-spec {:+.0}pp | copy {:+.0}pp | alloc {:+.0}pp | decode {:+.0}pp\n",
                subject.tool,
                other.tool,
                subject.topdown.backend_bound - other.topdown.backend_bound,
                subject.topdown.bad_speculation - other.topdown.bad_speculation,
                subject.bucket(FuncBucket::Copy) - other.bucket(FuncBucket::Copy),
                subject.bucket(FuncBucket::Alloc) - other.bucket(FuncBucket::Alloc),
                subject.bucket(FuncBucket::Decode) - other.bucket(FuncBucket::Decode),
            ));
        }
    }
    s
