...
//! mechanistic layers over the configured candidate regions into ONE ranked
//! lever list — the FULCRUM deliverable.
//!
//! Coz elasticity is the PRIMARY key (it directly answers ∂wall/∂speed). The
//! critical-path on-path share is a corroborating signal: a region with high
//! elasticity AND high on-path time is a confident lever; high elasticity but
//! ~0 on-path time is suspicious and flagged. The mechanism string tells you
//! HOW to attack the top lever.
...
pub struct Lever {
    ...
    /// elasticity but vastly different on-path share; this product is what
    /// actually ranks them, and is exactly why FULCRUM fuses the causal AND
    /// critical-path layers rather than trusting elasticity alone (the
    /// CPU-sum-lie trap in reverse: a high elasticity off the critical path is
    /// a small lever).
    pub lever_score: f64,
...
/// On-path fraction per region, summed from the critical-path entries (using
/// the config to map each critpath span label back to a region).
pub fn on_path_by_region(crit: &CritPath, cfg: &Config) -> BTreeMap<String, f64> {
    let mut m: BTreeMap<String, f64> = BTreeMap::new();
    for e in &crit.entries {
        if let Some(r) = cfg.label_region(&e.label) {
            *m.entry(r).or_default() += e.fraction;
        }
    }
    m
}

/// Produce the ranked lever list. `coz` may be None (critpath-only mode), in
/// which case ranking falls back to on-path fraction.
pub fn rank(
    ...
    let on_path = on_path_by_region(crit, cfg);
    let mut levers = Vec::new();

    for region_def in &cfg.regions {
        let region = &region_def.name;
        // The lever score uses the PEAK-line elasticity (the single highest-
        ...
        // same region. The median is kept as the CI-context band.
        ...
        // THE lever score: expected wall move = elasticity × on-path share. A
        // NaN elasticity (no coz signal) falls back to on-path alone (a region
        // that gates the wall is a lever even if coz couldn't sample it), so a
        // heavily on-path region never ranks below an off-path one just for
        // lack of coz experiments.
        ...
        let mechanism = mech
            .map(|m| m.region_mechanism(&region_def.functions))
    ...
    // is the fusion that lets a high-on-path / moderate-elasticity region
    // out-rank a high-elasticity / off-path one — the critical-path layer
    // disambiguating similar elasticities.
    levers.sort_by(|a, b| {
        b.lever_score
...
pub fn render(levers: &[Lever]) -> String {
    ...
    for (i, l) in levers.iter().enumerate() {
        ...
        // Trim the mechanism's verbose funcs[...] list for the table; the full
        // mechanism stays available in the Lever struct.
        let mech_short = l
            .mechanism
            .split(" | funcs[")
            .next()
    ...
    if let Some(top) = levers.first() {
        s.push_str(&format!(
            "\n  NEXT LEVER -> {}  (lever-score {:.3} = elasticity {} x {:.0}% on-path)\n   {}\n",
            top.region,
            top.lever_score,
