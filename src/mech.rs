...
//!
//! Each ranked lever carries a mechanism so the next move is obvious: a
//! DRAM-bound region wants prefetch / footprint shrink; a branch-miss region
//! wants a flatter hot path; a false-sharing region wants padding or
//! per-thread state.
//!
//! Attribution is FUNCTION-LEVEL (via perf's own symbol resolution), not a
//! per-span `(tid, timestamp)` join — that join is fragile (trace tids ≠ OS
//! tids; clock-base offset) and is a documented stretch goal. Function-level
//! is robust and already answers "is this region memory-bound or
//! branch-bound", because each configured region maps to distinct hot
//! functions.
//!
//! Three captures (all parsed from perf's TEXT/CSV output, not the binary
//! `perf.data`, so it's robust across perf versions):
//! 1. TMA top-down via `perf stat --topdown` → the dominant bound (Frontend /
//!    Backend / Retiring / BadSpec) for the whole run.
//! 2. PEBS memory profile (`perf mem record` / `perf record -e mem_load...`)
//!    → per-function DRAM/L3 share.
...
impl Mech {
    /// Summarize the mechanism for the function(s) backing a region. Pass the
    /// substrings that identify the region's functions (from the config's
    /// region `functions`).
    pub fn region_mechanism(&self, func_substrings: &[String]) -> String {
        let mut cycles = 0.0;
        let mut dram = 0.0;
        let mut hitm = 0.0;
        let mut matched = Vec::new();
        for (name, m) in &self.by_func {
            if func_substrings.iter().any(|s| name.contains(s.as_str())) {
                cycles += m.cycles_pct;
                dram += m.dram_pct;
                hitm += m.hitm;
...
/// target. Output (`-o csv_out`) is parsed by [`parse_topdown`].
pub fn perf_topdown_command(
    ...
    cmd.arg("stat")
        ...
        // explicit events if the kernel lacks it.
        .arg("--topdown");
    if let Some(c) = cpus {
        cmd.arg("--cpu").arg(c);
    }
    cmd.arg("--").arg(target_bin);
...
///
/// Modern `perf stat --topdown` (perf 6.x) emits a space-aligned HEADER row
/// naming the metrics in column order, then a DATA row of the percentages —
/// NOT a per-line keyword+value CSV:
///
...
/// metric names), record the metric ORDER, then (2) map the next row's floats
/// positionally. A fallback also handles the older same-line CSV form
/// (`,,12.3,,topdown-retiring`).
...
/// Parse a `perf report --stdio -n` overhead table → function → cycles_pct.
/// Lines look like: `    12.34%   1234   my_prog  [.] decode_block`.
