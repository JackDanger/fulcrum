...
src/xtool.rs      Cross-tool region accounting: profiles competing implementations
                  (rapidgzip, libdeflate, etc.) at comparable granularity on the same
                  inputs, normalizes TMA shapes side-by-side so a lever recommendation
                  can cite the gap rather than assert it.
